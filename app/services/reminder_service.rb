class ReminderService
  ...
  def self.calc_next_time(reminder, base_time=Time.current)
    notification_time = reminder.notification_datetime.strftime('%H:%M')

    case reminder.repeat_type
    when RepeatType.find_by(name: 'repeat-daily') then
      next_time = self.daily_next_time(notification_time)
    when RepeatType.find_by(name: 'repeat-weekly') then
  ...
  # 指定曜日・指定時刻の直近の日時
  def self.weekly_next_time(time_str, weekdays, now=Time.current)
    ...
    next_time = weekdays.map{ |weekday|
      time_of_day(
        time_str,
        base_day.next_occurring(WDAYS[weekday.id]) # 直近の指定曜日(基準日と同じ曜日なら1週間後)
      )
